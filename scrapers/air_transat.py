...
DEST_MAP = {
    ...
    "JM": "jamaica", "BB": "barbados", "AG": "antigua",
    "LC": "saint-lucia", "CR": "costa-rica",
}
        ...
        async with async_playwright() as p:
            ...
            for dest_code in dests_to_try:
                ...
                for nights in [7, 10, 14]:
                    try:
                        url = (
                            f"{self.base_url}/en/vacations/packages/{dest_slug}"
                            f"?departureDate={checkin}"
                            f"&duration={nights}"
                            f"&departure=YUL"
                        )
                        self.log.info("AirTransat URL: %s", url)
                        ...
                        self.log.info("AirTransat %s %dn: found %d cards", dest_code, nights, len(cards))
                        ...
                        for i, card in enumerate(cards[:20]):
                            try:
                                ...
                                deal = self._make_deal(
                                    ...
                                    url=link,
                                    source_trip_id=f"at_{dest_code}_{nights}n_{i}",
                                )
                                if price > 0:
                                    deals.append(deal)
                            except Exception as e:
                                self.log.debug("AirTransat card %d error: %s", i, e)
                                continue

                    except Exception as e:
                        self.log.error("AirTransat failed %s %dn: %s", dest_code, nights, e)
                        continue

            await browser.close()

        self.log.info("AirTransat: %d total deals", len(deals))
        return deals
