...
class WestJetScraper(BaseScraper):
    ...
    async def search(
        ...
        async with async_playwright() as p:
            ...
            try:
                url = f"{self.base_url}/vacations"
                await page.goto(url, timeout=30000, wait_until="networkidle")
                await page.wait_for_timeout(5000)

                cards = await page.query_selector_all('.vacation-card, .product-card, [class*="deal"], [class*="offer"], [class*="package"]')
                self.log.info("Found %d cards on WestJet", len(cards))

                for i, card in enumerate(cards[:50]):
                    try:
                        title_el = await card.query_selector('h3, h4, .hotel-name, [class*="title"], [class*="name"]')
                        title = await title_el.inner_text() if title_el else f"WestJet Hotel {i+1}"
                        ...
                        deal = self._make_deal(
                            ...
                            url=link,
                            source_trip_id=f"wj_{i}_{destination or 'all'}",
                        )
                        if price > 0:
                            deals.append(deal)
                ...
                self.log.error("WestJet scrape failed: %s", e)
            finally:
                await browser.close()

        self.log.info("WestJet: %d deals found", len(deals))
        return deals
